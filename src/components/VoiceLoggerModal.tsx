...
import { VoiceExpenseParseResult, SplitType, ExpenseCategory } from '../types';
import { 
  Mic, MicOff, Sparkles, Check, RefreshCw, X, ArrowRight, 
  Volume2, AlertCircle, FileText, DollarSign 
} from 'lucide-react';

export const VoiceLoggerModal: React.FC = () => {
  const { 
    isVoiceModalOpen, 
    setIsVoiceModalOpen, 
    activeGroup, 
  ...
  const [parsedResult, setParsedResult] = useState<VoiceExpenseParseResult | null>(null);
  ...
  const handleParseTranscript = async (textToParse?: string) => {
    ...
    try {
      ...
      if (data.parsed) {
        ...
        setCategory((p.category as ExpenseCategory) || 'Food & Dining');
        setSplitType((p.splitType as SplitType) || 'equal');
