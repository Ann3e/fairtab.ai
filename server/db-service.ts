...
import { Group, Expense, Settlement, RecurringRule, Dispute, GroupMessage, ActivityLog, Member, ExpenseCategory } from '../src/types';
...
export async function getAllGroups(): Promise<Group[]> {
  try {
    ...
    return allGroups.map(g => {
      ...
      const fallbackMember: Member = allMembersList[0] || {
        id: 'usr_alex',
        name: 'Alex Rivera',
      ...
      return {
        ...
        category: (g.category as any) || 'trip',
...
export async function updateGroup(groupId: string, updateData: Partial<Group>): Promise<Group | null> {
...
export async function joinGroupByCode(inviteCode: string, memberId: string): Promise<{ group: Group; member: Member } | null> {
  ...
  try {
    ...
    return rawExpenses.map(e => {
      ...
      const itemsForExp = rawItems.filter(i => i.expenseId === e.id).map(i => ({
        ...
        assignedMemberIds: (i.assignedMemberIds as string[]) || [],
      }));
      ...
      return {
        ...
        currency: e.currency,
        category: (e.category as ExpenseCategory) || 'Food & Dining',
        paidById: e.paidById,
        date: e.date,
        splitType: (e.splitType as any) || 'equal',
        splits: splitsForExp,
        ...
        recurringInterval: (e.recurringInterval as any) || undefined,
        disputeStatus: (e.disputeStatus as any) || 'none',
...
export async function createExpense(groupId: string, data: any): Promise<Expense> {
...
export async function updateExpense(groupId: string, expId: string, data: any): Promise<Expense | null> {
...
export async function deleteExpense(groupId: string, expId: string): Promise<boolean> {
...
export async function createSettlement(groupId: string, data: any): Promise<Settlement> {
...
export async function getRecurringRulesByGroupId(groupId: string): Promise<RecurringRule[]> {
  try {
    ...
    return list.map(r => ({
      ...
      currency: r.currency,
      category: (r.category as ExpenseCategory) || 'Utilities & Bills',
      paidById: r.paidById,
      splitType: (r.splitType as any) || 'equal',
      splits: (r.splitsJson as any) || [],
      interval: (r.interval as any) || 'monthly',
...
export async function createRecurringRule(groupId: string, data: any): Promise<RecurringRule> {
...
export async function getDisputesByGroupId(groupId: string): Promise<Dispute[]> {
  try {
...
export async function createDispute(groupId: string, data: any): Promise<Dispute> {
...
export async function addDisputeComment(groupId: string, dispId: string, memberId: string, text: string) {
...
export async function resolveDispute(groupId: string, dispId: string, status: string, updatedSplits?: any[]) {
...
export async function createGroupMessage(groupId: string, data: any): Promise<GroupMessage> {
...
export async function getActivityLogs(groupId: string): Promise<ActivityLog[]> {
  try {
